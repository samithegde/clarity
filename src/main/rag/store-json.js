...
function getIndexPath() {
  return (
    process.env.RAG_JSON_INDEX_PATH?.trim() ||
...
function search(queryEmbedding, { topK = 5, collection, minScore } = {}) {
  const index = loadIndex();
  ...
  return chunks
    ...
    .filter((chunk) => minScore == null || chunk.score >= minScore)
...
function getStats() {
  ...
  return { totalChunks: index.chunks.length, collections, store: "json" };
}
